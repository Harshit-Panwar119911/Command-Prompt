...
import java.io.*;
import java.text.*;
class NetDemo extends Panel implements KeyListener
{
...
protected void commandControl(KeyEvent ke)
{
			...
			else if(commandEnter.equalsIgnoreCase("dir"))
			{
				...
				//////////////////Change Directory/////////////////////
					...
					try
					{
						...
						if(cd.exists()==false)ta.append("Error:can't execute command'cd'\n");
						}
						catch(Exception e)
	...
	else if(commandEnter.startsWith("copy")||commandEnter.startsWith("copy"))
	{
		...
		else if(commandEnter.startsWith("ren")||commandEnter.startsWith("ren"))
		{
			...
			try
			{
				...
				if(oldFile.exists()==false)ta.append("Error:File'"+oldFile+"'does not exist\n");
				token=tokenizer.nextToken();
				...
				{
				ta.append("Error:can execute 'ren' command\n");
					}
					ta.append("\n");
					ta.append(currentPath+prompt);
					ta.setEditable(false);
	   ...
	   while(tokenizer.hasMoreTokens())
   {
	...
	else if(commandEnter.equalsIgnoreCase("memcheck"))
	{
		...
		ta.append("Inital free Memory:"+r.freeMemory()+"bytes"+"\n");
		r.gc();
		mem1=r.freeMemory();
	...
	//////////////////////////////////Prompt//////////////////////////////////
	else if(commandEnter.startsWith("propmt")||commandEnter.startsWith("prompt"))
	{
		ta.setEditable(true);
		ta.append(commandEnter+"\n");
		try
	...
	else if(commandEnter.equalsIgnoreCase("help")||commandEnter.equalsIgnoreCase("?"))
	{
		...
		ta.append("kill     |Deletes files        |kill file1.txt file2.txt\n");
		...
		ta.append("osinfo     |OS Information        |show readme.txt\n");
